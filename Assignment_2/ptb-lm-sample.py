...
def generate_samples(model, data):
    ...
    with torch.no_grad():
        ...
        samples = model.generate(inputs[0], hidden, model.seq_len, id_2_word).cpu().numpy()
        
    for s in range(samples.shape[1]):
        print('Sample '+str(s+1)+': ', end='')
        for t in range(samples.shape[0]):
            print(id_2_word[samples[t,s]], end=' ')
        print('\n')
...
if args.sample:
    generate_samples(model, valid_data)
else:
    raise Exception
