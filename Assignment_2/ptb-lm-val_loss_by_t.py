...
def get_val_loss_by_t(model, data, lr=1.0):
    """
    One epoch of validation averaged over all batches.
    """
    model.eval()    
    losses_by_t = []
    ...
    with torch.no_grad():
        ...
        for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
            ...
            # tt = torch.squeeze(targets.view(-1, model.batch_size * model.seq_len))
            tt_by_t = torch.squeeze(targets.view(-1, model.batch_size))
            ...
            # This line computes loss per time-step in the sequence
            loss_by_t = [loss_fn(outputs.contiguous().view(-1, model.batch_size, model.vocab_size)[t], tt_by_t[t]).data.item() for t in range(model.seq_len)]
                      
            losses_by_t.append(np.array(loss_by_t))
        
        # returns the loss per time-step averaged over all batches
        return np.mean(losses_by_t, axis = 0)



###############################################################################
# 
# Get Validation loss by time-step
#
###############################################################################
if args.evaluate:
    ...
    raise Exception('state_dict of trained model required')


print("\n########## Obtaining Validation loss by time-step ##################")
val_loss_by_t = get_val_loss_by_t(model, valid_data)

# SAVE loss by time-step 
loss_by_t_path = os.path.join(args.save_dir, 'loss_by_t_{}.npy'.format(args.model))
print('\nDONE\n\nSaving loss values by time-step to ' + loss_by_t_path)
np.save(loss_by_t_path, val_loss_by_t)
